...
- **Database**: PostgreSQL (accounts, password history, email verification/reset tokens)
...
3. Run the schema migration against your database: `psql -f backend/migrate_v2.sql` (expects a `users` table with `id`/`password_hash` to already exist - see the SQL for the base schema it assumes).
